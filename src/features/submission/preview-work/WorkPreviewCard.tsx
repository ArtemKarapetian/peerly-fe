...
export function WorkPreviewCard({
  ...
  return (
    <div className="bg-card border-2 border-border rounded-lg p-4 desktop:p-6">
      <div className="flex items-center justify-between mb-4 pb-4 border-b border-border">
        <h3 className="text-lg desktop:text-xl font-medium text-foreground tracking-[-0.5px]">
          {t("feature.workPreview.title")}
        </h3>
        {onOpenInNewWindow && (
          <button
            onClick={onOpenInNewWindow}
            className="inline-flex items-center gap-1.5 text-13 text-brand-primary hover:text-brand-primary transition-colors"
          >
      ...
      <div className="mb-4">
        ...
        <div className="space-y-2">
          {files.map((file) => (
            <div key={file.id} className="flex items-center gap-2 bg-muted rounded-sm px-3 py-2.5">
          ...
          <div className="bg-muted rounded-md p-3">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              {checksSummary.failed > 0 && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-13 text-destructive font-medium">
            ...
            <div className="mt-3 space-y-2">
              {validationChecks.map((check) => (
                <div key={check.id} className="flex items-start gap-2 text-13">
                  {check.status === "passed" && (
