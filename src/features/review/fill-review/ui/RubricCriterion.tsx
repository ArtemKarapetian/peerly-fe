    ...
    >
      ...
      <div className="mb-3">
        <div className="flex items-start justify-between gap-2 mb-1">
          <h4 className="text-15 desktop:text-base font-medium text-foreground">
            {criterion.name}
        ...
        {criterion.description && (
          <p className="text-13 text-muted-foreground leading-[1.4]">{criterion.description}</p>
        )}
      </div>
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-2">
          <label className="block text-13 text-muted-foreground">
            {t("feature.rubricCriterion.score")}
          </label>
          {!readonly && (
            <span className="text-2xs text-text-tertiary italic">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {scoreOptions.map((score) => (
            <button
              ...
              disabled={readonly}
              className={`
                w-10 h-10 rounded-sm text-sm font-medium transition-all
                ${
                  value.score === score
      ...
      {/* Comment Field */}
      <div className="mb-2">
        <label className="block text-13 text-muted-foreground mb-2">
        ...
        <textarea
          ...
          className={`
            w-full px-3 py-2 border-2 rounded-sm text-sm text-foreground 
      ...
      {/* Error */}
      {error && (
        <div className="flex items-start gap-2 bg-error-light border border-error rounded-sm p-2 mb-2">
