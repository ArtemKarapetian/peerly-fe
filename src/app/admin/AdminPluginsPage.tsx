...
export default function AdminPluginsPage() {
  ...
  const [configModal, setConfigModal] = useState<{ plugin: Plugin; config: Record<string, string> } | null>(null);
  ...
  const [plugins, setPlugins] = useState<Plugin[]>([
    {
      ...
      description: 'Автоматическая проверка работ на плагиат с использованием алгоритмов сравнения текстов',
      version: '2.1.0',
      ...
      configSchema: [
        { key: 'apiKey', label: 'API Key', type: 'text', required: true, placeholder: 'Введите ключ API' },
        { key: 'threshold', label: 'Порог совпадения (%)', type: 'number', required: true, validation: { min: 0, max: 100 } },
    ...
    },
    {
      ...
      configSchema: [
        { key: 'maxSize', label: 'Максимальный размер (MB)', type: 'number', required: true, validation: { min: 1, max: 100 } },
        { key: 'allowedFormats', label: 'Разрешённые форматы', type: 'text', required: true, placeholder: 'pdf,docx,txt' },
    ...
    },
    {
      id: 'anonymizer',
      name: 'Anonymizer',
      description: 'Автоматическая анонимизация имён и идентификаторов в работах для честного peer review',
      version: '2.0.3',
      category: 'Privacy',
      status: 'healthy',
      installed: true,
      ...
      configSchema: [
        ...
        { key: 'preserveMetadata', label: 'Сохранять метаданные', type: 'boolean', required: false }
      ]
    },
    {
      id: 'toxicity-checker',
    ...
    // Available plugins (not installed)
    {
      ...
      installed: false,
      configSchema: [
        { key: 'model', label: 'Модель ML', type: 'select', required: true, options: ['gpt-4', 'claude', 'local'] },
        { key: 'confidence', label: 'Минимальная уверенность (%)', type: 'number', required: true, validation: { min: 0, max: 100 } }
      ]
    },
    {
      ...
      installed: false,
      configSchema: [
        { key: 'webhookUrl', label: 'Webhook URL', type: 'text', required: true, placeholder: 'https://hooks.slack.com/...' },
        { key: 'channel', label: 'Канал по умолчанию', type: 'text', required: true, placeholder: '#general' }
      ]
    },
    {
      id: 'citation-checker',
      ...
      installed: false,
      configSchema: [
        { key: 'style', label: 'Стиль цитирования', type: 'select', required: true, options: ['APA', 'MLA', 'Chicago', 'GOST'] }
      ]
    }
  ]);
  ...
  const handleUpdate = (plugin: Plugin) => {
    ...
    setPlugins(plugins.map(p => 
      p.id === plugin.id 
        ? { ...p, version: newVersion, lastRun: new Date() }
        : p
    ));
  ...
  const renderPluginCard = (plugin: Plugin) => (
    <div key={plugin.id} className="bg-white border-2 border-[#e6e8ee] rounded-[20px] p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)] transition-all">
      {/* Header */}
                    ...
                    {field.type === 'boolean' && (
                      <label className="flex items-center gap-3 p-4 border-2 border-[#e6e8ee] rounded-[12px] cursor-pointer hover:bg-[#f9f9f9] transition-colors">
                        <input
                          ...
                          onChange={(e) => setConfigModal({
                            ...configModal,
                            config: { ...configModal.config, [field.key]: e.target.checked ? 'true' : 'false' }
                          })}
