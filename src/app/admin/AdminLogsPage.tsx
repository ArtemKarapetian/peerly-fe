...
// CSS class constants
const CLASSES = {
  input: 'w-full px-4 py-3 border-2 border-[#e6e8ee] rounded-[12px] text-[15px] text-[#21214f] focus:border-[#2563eb] focus:outline-none transition-colors',
  ...
  filterTag: 'inline-flex items-center gap-1 px-2 py-1 bg-[#f9f9f9] text-[#21214f] rounded-[6px] text-[12px]',
};
...
export default function AdminLogsPage() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setAuditLogs(parsed.map((log: any) => ({
        ...log,
  ...
  const handleExport = () => {
    const data = activeTab === 'system' ? filteredSystemLogs : filteredAuditLogs;
    const csvContent = activeTab === 'system'
      ? 'Timestamp,Service,Level,Message,Details\n' + 
        data.map(log => 
          `${log.timestamp.toISOString()},${log.service},${(log as SystemLog).level},"${log.message}","${(log as SystemLog).details || ''}"`
        ).join('\n')
      : 'Timestamp,User,Action,Resource,Details,IP\n' + 
        data.map(log => {
          const auditLog = log as AuditLog;
          return `${log.timestamp.toISOString()},${auditLog.userName},${auditLog.action},${auditLog.resource},"${auditLog.details}",${auditLog.ipAddress || ''}`;
        }).join('\n');
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredSystemLogs.map((log, index) => (
                      <tr key={log.id} className={`border-b border-[#e6e8ee] ${index % 2 === 0 ? 'bg-white' : 'bg-[#fafafa]'}`}>
                        ...
                        <td className="px-4 py-3 text-[13px] text-[#21214f]">
                          {log.message}
                        </td>
          ...
          <div className="space-y-3">
            {filteredAuditLogs.length > 0 ? (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[13px] text-[#767692] mb-2">
                        <span className="font-medium text-[#21214f]">{log.resource}</span>: {log.details}
                      </p>
              ...
              <EmptyState
                ...
                description={searchQuery ? 'Попробуйте изменить параметры поиска' : 'Записи аудита появятся после действий администраторов'}
              />
            )}
          </div>
        )}
