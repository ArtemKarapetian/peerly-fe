...
export default function TeacherAppealsPage() {
  ...
  const generateAppeals = (): Appeal[] => {
    ...
    reviews.slice(0, 8).forEach((review, idx) => {
      ...
      const avgScore = Object.values(review.scores).reduce((sum, s) => sum + s, 0) / Object.values(review.scores).length;
      ...
      if (status === 'resolved') {
        const resolutions: ('approved' | 'denied' | 'adjusted')[] = ['approved', 'denied', 'adjusted'];
        ...
        if (appeal.resolution === 'approved') {
          ...
          appeal.teacherResponse = 'Апелляция одобрена. Оценка пересмотрена в соответствии с критериями.';
          ...
          appeal.teacherResponse = 'После рассмотрения апелляции решено сохранить исходную оценку. Рецензент корректно применил критерии рубрики.';
        }
      }
  ...
  const getStatusBadge = (status: AppealStatus) => {
    switch (status) {
      case 'new':
        return <span className="inline-flex items-center gap-1 px-2 py-1 bg-[#e9f5ff] text-[#5b8def] rounded-[6px] text-[12px] font-medium">
  ...
  const handleMarkInReview = (appeal: Appeal) => {
    setAppeals(prev => prev.map(a => 
      a.id === appeal.id ? { ...a, status: 'in_review' as AppealStatus } : a
    ));
  };
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white border-2 border-[#e6e8ee] rounded-[12px] p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[12px] text-[#767692] uppercase tracking-wide">На рассмотрении</span>
            </div>
                  ...
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-[13px] text-[#767692]">
                        ...
                        <span>
                          Оценка: <strong className="text-[#21214f]">{appeal.originalScore.toFixed(1)}</strong>
                        </span>
                        {appeal.requestedScore && (
                          <span>
                            Запрошено: <strong className="text-[#5b8def]">{appeal.requestedScore.toFixed(1)}</strong>
                          </span>
                        )}
                        {appeal.newScore && (
                          <span>
                            Новая оценка: <strong className="text-[#4caf50]">{appeal.newScore.toFixed(1)}</strong>
                          </span>
                        )}
                      </div>
                    </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-[#f9f9f9] border-2 border-[#e6e8ee] rounded-[12px] p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[12px] text-[#767692] uppercase tracking-wide mb-1">Задание</p>
                    <p className="text-[14px] text-[#21214f] font-medium">{selectedAppeal.assignmentTitle}</p>
                  </div>
                  <div>
                    <p className="text-[12px] text-[#767692] uppercase tracking-wide mb-1">Студент</p>
                    <p className="text-[14px] text-[#21214f] font-medium">{selectedAppeal.studentName}</p>
                  </div>
                  <div>
                    <p className="text-[12px] text-[#767692] uppercase tracking-wide mb-1">Исходная оценка</p>
                    <p className="text-[20px] text-[#d4183d] font-medium">{selectedAppeal.originalScore.toFixed(1)}/5</p>
                  </div>
                  {selectedAppeal.requestedScore && (
                    <div>
                      <p className="text-[12px] text-[#767692] uppercase tracking-wide mb-1">Запрашиваемая оценка</p>
                      <p className="text-[20px] text-[#5b8def] font-medium">{selectedAppeal.requestedScore.toFixed(1)}/5</p>
                    </div>
                  )}
                  {selectedAppeal.newScore && (
                    <div>
                      <p className="text-[12px] text-[#767692] uppercase tracking-wide mb-1">Новая оценка</p>
                      <p className="text-[20px] text-[#4caf50] font-medium">{selectedAppeal.newScore.toFixed(1)}/5</p>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      <span className="text-[15px] font-medium">
                        Одобрить апелляцию
                        {selectedAppeal.requestedScore && ` (новая оценка: ${selectedAppeal.requestedScore.toFixed(1)})`}
                      </span>
                    </button>
