...
export function TeacherCourseAssignments({ courseId }: TeacherCourseAssignmentsProps) {
  const assignments = demoDataStore.getAssignmentsByCourse(courseId);

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'published':
        return <span className="px-2 py-1 bg-[#e8f5e9] text-[#4caf50] rounded-[6px] text-[12px] font-medium">Опубликовано</span>;
      case 'draft':
        return <span className="px-2 py-1 bg-[#f5f5f5] text-[#767692] rounded-[6px] text-[12px] font-medium">Черновик</span>;
      case 'closed':
        return <span className="px-2 py-1 bg-[#fff5f5] text-[#d4183d] rounded-[6px] text-[12px] font-medium">Закрыто</span>;
      default:
        return null;
    }
  };
        ...
        {assignments.map((assignment, index) => (
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <h3 className="text-[17px] font-medium text-[#21214f]">
                {assignment.title}
              </h3>
              {getStatusBadge(assignment.status)}
            </div>
            <p className="text-[14px] text-[#767692] mb-3">
