...
export function TeacherCourseAnnouncements({ courseId }: TeacherCourseAnnouncementsProps) {
  const announcements = demoDataStore.getAnnouncements().filter(a => a.courseId === courseId);

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <p className="text-[15px] text-[#767692]">
          Объявления для студентов курса
        </p>
      ...
      <div className="space-y-3">
        {announcements.map((announcement) => (
          <div
            key={announcement.id}
            className="border-2 border-[#e6e8ee] rounded-[12px] p-4"
          >
