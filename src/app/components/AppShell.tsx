...
export function AppShell({ children, title }: AppShellProps) {
  ...
  const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(false); // Used for both desktop and tablet
  ...
  // Default tablet to collapsed on initial load
  useEffect(() => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-white">
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {isMobile && (
          <TopBar onMenuClick={() => setIsMobileMenuOpen(true)} title={title} />
        )}
