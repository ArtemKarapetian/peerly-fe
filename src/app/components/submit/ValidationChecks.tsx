...
export function ValidationChecks({ checks }: ValidationChecksProps) {
  ...
  const getStatusBgColor = (status: CheckStatus) => {
    switch (status) {
      case 'queued':
        return 'bg-[#f9f9f9]';
      case 'running':
    ...
    return (
      <div className="text-center py-6">
        <p className="text-[13px] text-[#767692]">
          Проверки будут запущены после загрузки файла
        </p>
      </div>
    );
  }
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2 mb-1">
                <div className="text-[14px] font-medium text-[#21214f]">
                  {check.name}
                </div>
              ...
              <div className="text-[12px] text-[#767692] leading-[1.4]">
                {check.description}
              </div>
              {check.message && (
