 ...
 */
export function userExists(username: string, email: string): { exists: boolean; field?: 'username' | 'email' } {
...
export function authenticateUser(identifier: string, password: string): StoredUser | null {
  ...
  const user = users.find(u => 
    (u.username.toLowerCase() === normalizedIdentifier || u.email.toLowerCase() === normalizedIdentifier) &&
    u.password === password
  );
