...
const DEMO_RULES: AutomationRule[] = [
  {
    ...
    conditions: [
      { type: 'review_comment_length', operator: '<', value: 50 }
    ],
    actions: [
      { type: 'flag_review' },
  ...
  },
  {
    ...
    conditions: [
      { type: 'plagiarism_score', operator: '>', value: 70 }
    ],
    actions: [
      { type: 'flag_submission' },
      { type: 'require_moderation' }
    ],
    scope: { applyToAll: true },
  ...
  },
  {
    ...
    trigger: 'SubmissionUploaded',
    conditions: [
      { type: 'submission_delay_days', operator: '>', value: 0 }
    ],
    actions: [
      { type: 'apply_penalty', parameters: { penaltyPercent: 10 } }
    ],
    scope: { applyToAll: true },
...
export function createRule(rule: Omit<AutomationRule, 'id' | 'createdAt' | 'updatedAt'>): AutomationRule {
...
export function testRule(rule: AutomationRule): { matches: number; samples: string[] } {
  // Demo simulation
