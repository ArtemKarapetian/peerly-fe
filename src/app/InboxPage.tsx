...
type NotificationType = 'DEADLINE' | 'REVIEW_ASSIGNED' | 'REVIEW_RECEIVED' | 'GRADE_PUBLISHED' | 'COMMENT' | 'TASK_UPDATED';
                    ...
                    {(Object.keys(filterLabels) as FilterType[]).map((filter) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 text-[14px] hover:bg-[#f9f9f9] transition-colors ${
                          selectedFilter === filter ? 'bg-[#f0f4ff] text-[#3d6bc6] font-medium' : 'text-[#21214f]'
                        }`}
                      >
                        {filterLabels[filter]}
                      </button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-[14px] text-[#767692] leading-[1.6]">
                  Здесь отображаются все важные события: дедлайны заданий, новые рецензии, комментарии
                  преподавателей и опубликованные оценки. Нажмите на уведомление, чтобы перейти к связанной странице.
                </p>
              </div>
            </div>
          </div>
